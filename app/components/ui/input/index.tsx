...
import { useCallback, useEffect, useRef, useState, type FC, type InputHTMLAttributes } from "react";
import styles from './Input.module.css'

export interface Props extends Omit<InputHTMLAttributes<HTMLInputElement>, 'onChange' | 'value' | 'type'> {
  variant?: Variant;
  infoPosition?: 'inside-right' | 'outside-left' | 'outside-right' | 'outside-top' | 'outside-bottom';
  labelText?: string;
...
const Input: FC<Props> = ({
  ...
  const [currentLabelTextAdditional, setCurrentLabelTextAdditional] = useState<string|undefined>(labelTextAdditional);
  ...
  const setInfoBtnState = (text?: string, type? :'result' | 'warning' | 'info') =>{
    ...
    return (
      <fieldset
        ...
        <input
          ...
          className={clsx(
            '',
            styles.input, styles[variant], className)}
          value={stringValue}
        ...
        infoBtn.text &&
        <span
          className={clsx(styles.inputInfo,  styles[variant], infoBtn.type && styles[infoBtn.type], className)}
